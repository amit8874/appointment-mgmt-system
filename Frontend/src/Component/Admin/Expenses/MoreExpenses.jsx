...
const MoreExpenses = () => {
  ...
  // Common category options for clinic owners
  const categorySuggestions = [
    'Doctor Visit / Fee',
    'Clinic Rent',
    'Electricity / Utilities',
    'Clinic Cleaning / Maintenance',
    'Staff Salary / Bonus',
    'Office Supplies / Stationery',
    'Refreshments / Snacks',
    'Marketing / Advertising',
    'Software / IT Subscription',
    'Other'
  ];
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      const res = await expenseApi.getOtherExpenses(params);
      setExpenses(res.expenses || []);
      setTotalPages(res.totalPages || 1);
      setTotalItems(res.total || 0);

      // Simple local KPI calculation based on filtered set
      ...
      toast.error('Failed to load general expenses list.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.expenseName) {
      toast.warning('Expense Name/Item Name is required.');
      return;
    }

    try {
      if (modalMode === 'add') {
        await expenseApi.createOtherExpense(formData);
        toast.success('Expense recorded successfully!');
      } else {
        await expenseApi.updateOtherExpense(selectedExpense._id, formData);
  ...
  const handleExportExcel = () => {
    ...
    const exportData = expenses.map((e, idx) => ({
      'S.No': idx + 1,
      'Expense/Item Name': e.expenseName,
      'Category': e.category,
      'Paid To / Vendor': e.vendor || '-',
      'Expense Date': e.purchaseDate ? new Date(e.purchaseDate).toLocaleDateString() : '-',
      'Invoice Number': e.invoiceNumber || '-',
      'Quantity': e.quantity,
      'Unit Cost (INR)': e.unitPrice,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'More Expenses');

    const dateStr = new Date().toISOString().split('T')[0];
    XLSX.writeFile(workbook, `More_Clinic_Expenses_${dateStr}.xlsx`);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const response = await expenseApi.downloadOtherPdf(params);
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `More_Expenses_${Date.now()}.pdf`);
    ...
    <div className="p-4 sm:p-6 space-y-6 bg-slate-50/50 dark:bg-gray-900/50 min-h-screen">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs font-bold text-slate-400 dark:text-gray-400 uppercase tracking-widest mt-1">
            Track utility bills, rent, doctor payments, cleaning & other custom outflows
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 p-5 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-gray-400 uppercase tracking-widest">Total Outflow</p>
        ...
        <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 p-5 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-gray-400 uppercase tracking-widest">Total Settled</p>
        ...
        <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 p-5 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-gray-400 uppercase tracking-widest">Total Unsettled/Due</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-xs">
            <thead className="bg-slate-50 dark:bg-gray-900/40 text-slate-500 dark:text-gray-400 border-b border-slate-200 dark:border-gray-700">
              <tr>
                <th className="p-4 font-black uppercase tracking-widest text-[10px]">Expense Name</th>
                <th className="p-4 font-black uppercase tracking-widest text-[10px]">Category</th>
                <th className="p-4 font-black uppercase tracking-widest text-[10px]">Paid To / Vendor</th>
                <th className="p-4 font-black uppercase tracking-widest text-[10px]">Expense Date</th>
              ...
              {loading ? (
                ...
                expenses.map((exp) => (
                  <tr key={exp._id} className="hover:bg-slate-50/50 dark:hover:bg-gray-700/30 transition-colors">
                    <td className="p-4 font-bold text-slate-800 dark:text-white">{exp.expenseName}</td>
                    <td className="p-4 font-medium text-slate-500">{exp.category}</td>
                ...
                <tr>
                  <td colSpan="9" className="p-8 text-center text-slate-400">
                    ...
                    <span>No general expenses recorded yet.</span>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* Record Vault segment at the bottom */}
      <RecordVault expenseType="More Expenses" />
            ...
            >
              ...
              <div className="p-6 border-b border-slate-100 dark:border-gray-700 flex items-center justify-between bg-slate-50 dark:bg-gray-900/20">
                <h3 className="text-sm font-black uppercase tracking-wider text-slate-800 dark:text-white flex items-center gap-2">
                  {modalMode === 'add' ? '💸 Add Clinic Expense' : modalMode === 'edit' ? '✏️ Edit Clinic Expense' : '👁️ View Expense Details'}
                </h3>
                <button
                  onClick={() => setIsModalOpen(false)}
              ...
              <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Expense/Item Name *</label>
                    <input
                      type="text"
                      name="expenseName"
                      ...
                      required
                      placeholder="e.g. Doctor Visit Fee, Electric Bill Jul, Clinic Rent"
                  ...
                  <div>
                    ...
                    {modalMode === 'view' ? (
                      <input
                        ...
                        disabled
                        className="w-full px-4 py-2 border border-slate-200 dark:border-gray-700 rounded-xl text-xs font-semibold bg-slate-50/50 dark:bg-gray-900/50 text-slate-800 dark:text-white"
                      />
                    ) : (
                      <select
                        name="category"
                  ...
                  {/* Custom Category fallback if 'Other' selected */}
                  {formData.category === 'Other' && modalMode !== 'view' && (
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Custom Category Name</label>
                      <input
                        type="text"
                        name="customCategory"
                        placeholder="e.g. Fire Safety, Water Bill"
                  ...
                  {/* Vendor / Paid To */}
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Paid To / Vendor</label>
                    <input
                      type="text"
                      name="vendor"
                      ...
                      onChange={handleInputChange}
                      disabled={modalMode === 'view'}
                      placeholder="e.g. Dr. Ramesh Kumar, Electricity Board"
                      className="w-full px-4 py-2 border border-slate-200 dark:border-gray-700 rounded-xl text-xs font-semibold outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-slate-50/50 dark:bg-gray-900/50 text-slate-800 dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Bill/Invoice Reference Number</label>
                    <input
                      type="text"
                      name="invoiceNumber"
                  ...
                  {/* Expense Date */}
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Expense Date *</label>
                    <input
                      type="date"
                      name="purchaseDate"
                  ...
                  {/* Quantity */}
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Quantity / Count</label>
                    <input
                      type="number"
                      name="quantity"
                      min="1"
                  ...
                  {/* Unit Price */}
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Rate / Cost per unit (INR)</label>
                    <input
                      type="number"
                      name="unitPrice"
                      min="0"
                  ...
                  {/* GST Amount */}
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">GST / Surcharges (INR)</label>
                    <input
                      type="number"
                      name="gstAmount"
                      min="0"
                  ...
                  {/* Notes */}
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Notes / Description</label>
                    <textarea
                      name="notes"
                      rows="2"
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Receipt / Invoice Document</label>
                    <div className="flex items-center gap-3">
                      {modalMode !== 'view' && (
                        <label className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-slate-700 dark:text-white rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer transition-all">
                          <Upload size={14} />
                          <span>{uploading ? 'Uploading...' : 'Upload Bill'}</span>
                          <input
                            type="file"
                      ...
                      {formData.invoiceUrl ? (
                        <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                          ...
                          <a href={formData.invoiceUrl} target="_blank" rel="noopener noreferrer" className="hover:underline truncate max-w-[200px]" title={fileName || 'View file'}>
                            {fileName || 'View Uploaded Document'}
                          </a>
                        </div>
                      ) : (
                        <span className="text-xs text-slate-400 dark:text-gray-500 italic">No receipt attached.</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Form Buttons */}
                <div className="pt-4 border-t border-slate-100 dark:border-gray-700 flex justify-end gap-3 no-print">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-4 py-2 border border-slate-200 dark:border-gray-700 hover:bg-slate-50 dark:hover:bg-gray-700 rounded-xl text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 transition-colors"
                  >
                    Close
                  </button>
                  {modalMode !== 'view' && (
                    <button
                      type="submit"
                      disabled={uploading}
                      className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50"
                    >
                      Save Expense
                    </button>
                  )}
