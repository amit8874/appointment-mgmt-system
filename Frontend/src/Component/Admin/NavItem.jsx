...
const NavItem = ({ id, name, icon: Icon, currentTab, onClick, toggleSidebar, isSidebarCollapsed }) => {
  ...
  return (
    ...
    >
      ...
      <Icon className={`w-5 h-5 z-10 ${isSidebarCollapsed ? '' : 'mr-3'} ${isActive ? 'text-indigo-600' : 'text-gray-400 dark:text-gray-500'}`} />
      {!isSidebarCollapsed && <span className="z-10 text-base whitespace-nowrap">{name}</span>}
    </button>
  );
};
