    ...
    const fetchData = async () => {
      try {
        ...
        setStats({
          ...
          monthlyRevenue: billing.totalCollected || 0,
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 w-full max-w-4xl rounded-none shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]"
      >
        <div className="p-8 border-b border-slate-100 dark:border-gray-700 flex justify-between items-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-5">
            <div className={`p-4 rounded-none ${report?.bg} shadow-soft`}>
              <report.icon className={`w-8 h-8 ${report?.color}`} />
            </div>
            <div>
              <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{report?.title}</h2>
              <p className="text-slate-500 font-medium italic text-sm">Generated for current period</p>
            </div>
          </div>
          <button onClick={onClose} className="p-3 hover:bg-slate-100 dark:hover:bg-gray-700 rounded-none transition-all">
        ...
        <div className="p-10 overflow-y-auto">
          ...
          <div className="h-80 w-full bg-slate-50 dark:bg-gray-700/30 rounded-none p-8 mb-10 border border-slate-100 dark:border-gray-700">
            ...
            {['revenue', 'performance', 'system', 'custom'].includes(report?.id) && (
          ...
          <div className="bg-blue-600 rounded-none p-8 flex items-center justify-between shadow-soft">
            <div>
              <h4 className="text-white text-xl font-black mb-1">Ready to export?</h4>
              <p className="text-blue-100 font-medium">Get a detailed PDF version of this report.</p>
            </div>
            <button className="bg-white text-blue-600 px-8 py-4 rounded-none font-black flex items-center gap-3 hover:scale-105 transition-transform active:scale-95 shadow-lg">
